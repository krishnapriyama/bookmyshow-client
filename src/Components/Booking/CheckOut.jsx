...
const ProductBillingPage = () => {
   ...
   const [checkoutDetieals, setDeteals] = useState(state)
               ...
               <div className="flex flex-col  xl:flex-row justify-center xl:justify-between space-y-6 xl:space-y-0 xl:space-x-6 w-full">
                  ...
                  <div className="p-8 bg-gray-100 flex flex-col lg:w-full xl:w-3/5">
                     ...
                     <div className="mt-8"> <span className="text-[#9a9e9b] ">Movie Name</span>
                        ...
                        >{checkoutDetieals?.movie?.moviename}   </span>
                     </div>
                     <div className="mt-2 flex-col"> <span className="text-[#9a9e9b] ">Theater</span>
                        <div >
                           <input
                              ...
                              disabled value={`${checkoutDetieals?.theater?.name} , ${checkoutDetieals?.theater?.screen.screenname}`}
                           />
                        </div>
                        <div> <span className="text-[#9a9e9b] ">Seats</span>
                           <input
                              className="border rounded-tl rounded-tr border-gray-300 p-4 w-full text-base leading-4 placeholder-gray-600 text-[#16572b]"
                              disabled value={checkoutDetieals?.seat}
                           />
                        </div><span className="text-[#9a9e9b] ">Payment</span>
                        <div className="flex-row flex mb-5 ">
                           <input
                              className="border rounded-bl border-gray-300 p-4 w-full text-base leading-4 placeholder-gray-600 text-gray-600"
                              disabled value={`${checkoutDetieals?.price} * ${checkoutDetieals?.seat?.length}`}
                           />
                           <input
                              className="border rounded-br border-gray-300 p-4 w-full text-base leading-4 placeholder-gray-600 text-[#16572b]"
                              disabled value={`${checkoutDetieals?.totelprice} INR`}
                           />
                        </div>
                     </div>
                     <Payment amount={checkoutDetieals?.totelprice} bookingid={checkoutDetieals?.Bookingid} />
                  </div>
               </div>
            </div>
         </div>
